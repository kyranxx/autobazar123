...
ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS notify_moderation_email BOOLEAN NOT NULL DEFAULT TRUE;

CREATE OR REPLACE FUNCTION public.is_seller_auto_publish_eligible(
  p_user_id UUID,
  p_description TEXT
)
RETURNS BOOLEAN
...
AS $$
DECLARE
  v_verified_dealer_id UUID;
  v_approved_ads_count INTEGER := 0;
...
BEGIN
  ...
  SELECT id
  INTO v_verified_dealer_id
  FROM public.dealers
  WHERE owner_id = p_user_id
    AND is_verified = TRUE
  LIMIT 1;

  IF v_verified_dealer_id IS NULL THEN
    RETURN FALSE;
  END IF;

  SELECT COUNT(*)
  INTO v_approved_ads_count
  FROM public.ads
  WHERE seller_id = p_user_id
    AND status IN ('active', 'sold');

  IF v_approved_ads_count < 5 THEN
    RETURN FALSE;
  END IF;

  IF char_length(v_description) > 1600 THEN
    RETURN FALSE;
  END IF;

  IF v_description ~* '(whatsapp|telegram|western union|wire transfer|crypto|gift card|prepayment|deposit in advance)' THEN
    RETURN FALSE;
  END IF;

  IF v_description ~* '(https?://|www\\.|@gmail\\.|@hotmail\\.|@outlook\\.)' THEN
    RETURN FALSE;
  END IF;

  IF v_description ~ '([!?.,])\\1{4,}' THEN
    RETURN FALSE;
  END IF;

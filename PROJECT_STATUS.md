...
- Current main has a local root-cause fix for that prefetch noise: Next `_rsc` prefetch requests on protected routes no longer consume the protected-route rate-limit budget before redirect/RBAC handling. RED/GREEN evidence: new proxy regression first failed because `checkRateLimit` was called once, then passed after the fix; `npx vitest run src/proxy.test.ts` now passes 19/19. Support checks passed: `npm run test:security:release-gate`, `npm run typecheck`, `npm run lint`, and `git diff --check`. Not deployed yet.
- Still not public-launch ready: after Turnstile keys are configured, create/verify a clean reviewed source that includes `8a6f520` plus the local `_rsc` prefetch fix, then deploy and rerun deployed inquiry/browser smoke before dealer outreach.

## 2026-06-21 post-deploy/migration/payment update

- Last deployed app source was `C:\Users\User\Desktop\Projects\ab123-rs-153336` at `2297260` (`fix: handle stripe failed payment intents`). The same clean detached reviewed worktree has now been advanced locally to `8a6f520` (`docs: record latest launch blocker evidence`) for the next reviewed-source checks; it has not been pushed or deployed. The main repo still has the separate dirty taxonomy lane and remains ahead of `origin/master`.
...
- Fresh latest reviewed-source full rollup from `C:\Users\User\Desktop\Projects\ab123-rs-153336` at `8a6f520` ran `npm run check:launch-blockers:full -- --allow-extra-worktrees`. It is blocked only by missing Turnstile Vercel env metadata names. Passing lanes include branch/worktree state, deploy-source readiness, package-script tracking, cleanup/refactor scan, `git diff --check`, launch account coverage, security release gate, `npm audit`, payments 6 files / 53 tests, cron/email 10 files / 38 tests, fallbacks, Algolia parity 56/56, SEO/GEO 3 files / 31 tests, local smoke helper 4/4, uploads 3 files / 19 tests, performance budgets, lint, typecheck, build, production bundle budget, Upstash env checks, live RLS, PPR packaging, Vercel Preview local build preflight, and launch migration worktree safety. The reviewed worktree remained clean after the run.
...
- Fresh deployed external browser audit is currently not green on the deployed `2297260` source because protected-route `_rsc` prefetches can hit 429 on mobile. Current main fixes this locally, but it still needs reviewed-source isolation/deploy and a rerun of the external audit before claiming deployed browser/search audit green.
- Still not public-launch ready: crawler indexing is intentionally disabled; public SEO opening needs explicit owner approval after final SEO/canonical checks. Fix Turnstile envs, create/verify a clean reviewed source that includes the local SEO title/H1 fix and `_rsc` prefetch fix, then deploy and rerun deployed inquiry/browser smoke before dealer outreach.

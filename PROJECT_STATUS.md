...
- Local app-side Vercel preflight blockers were reduced, but preview packaging is still not green:
  ...
  - Passed: `node --test scripts/check-vercel-ppr-lambda-blocker.test.mjs`, 5/5. Passed expected-blocked diagnostic: `node scripts/check-vercel-ppr-lambda-blocker.mjs --expect-blocked`.
...
- Still launch-blocking:
  ...
  - Payment confirmation email job/provider/Gmail delivery is verified from an isolated current local webhook path, but remote `payment_notifications.billing_transaction_id` is still missing until the launch-critical payment migration is applied and rechecked.
...
- Still launch-blocking:
  ...
  - Payment notification schema drift is fixed locally in commit `0bbf14f`, but the migration is not deployed. Real payment confirmation email delivery is verified by an isolated current-webhook smoke; payment notification logging is not remote-ready until the migration is applied and rechecked.
...
1. Deploy RLS/payment-compatible code, safely apply remote profile/dealer/payment migrations using `docs/launch-remote-migration-deploy-runbook.md`, then rerun the live anon probe and recheck payment notification logging.
2. Smoke preview/production health, maintenance bypass, cron/email jobs, and core auth/listing/payment flows after deploy approval.

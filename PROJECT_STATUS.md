...
- Clean reviewed source `C:\Users\User\Desktop\Projects\ab123-rs-153336` is now advanced locally to `8d98f00`, including the SEO title/H1 fix, `_rsc` prefetch fix, Cloudflare upload smoke helper, dealer top-up smoke helper, and fresh Turnstile blocker/runbook docs. Fresh guards passed there: `npm run check:deploy-source-readiness`, `npm run check:launch-migration-worktree -- --root C:\Users\User\Desktop\Projects\ab123-rs-153336`, and `npm run check:launch-blockers:full -- --allow-extra-worktrees` with only the expected missing Turnstile Vercel env metadata blocker. Passing full-rollup lanes included package-script tracking for 115 paths, cleanup/refactor scan across 563 launch-code files, security release gate, `npm audit`, payments 53/53, cron/email 38/38, Algolia 56/56, SEO/GEO 32/32, uploads 19/19, performance, lint, typecheck, build, production bundle budget, Upstash env checks, live RLS, Vercel static-PPR packaging, Vercel Preview local build preflight, and launch migration worktree safety. The reviewed worktree stayed clean.
- Current main and clean reviewed source `8d98f00` have a local root-cause fix for the deployed prefetch noise: Next `_rsc` prefetch requests on protected routes no longer consume the protected-route rate-limit budget before redirect/RBAC handling. RED/GREEN evidence from the earlier `68782c2` fix remains: new proxy regression first failed because `checkRateLimit` was called once, then passed after the fix; `npx vitest run src/proxy.test.ts` passes 19/19 inside the fresh full rollup. Not deployed yet.
- Still not public-launch ready: after Turnstile keys are configured, deploy clean reviewed source `8d98f00`, then rerun deployed inquiry/browser smoke and the external browser audit before dealer outreach.

## 2026-06-21 post-deploy/migration/payment update

- Last deployed app source was `C:\Users\User\Desktop\Projects\ab123-rs-153336` at `2297260` (`fix: handle stripe failed payment intents`). The same clean detached reviewed worktree has now been advanced locally to `8d98f00` (`docs: clarify turnstile launch blocker`) for the next reviewed-source checks; it has not been pushed or deployed. The main repo still has the separate dirty taxonomy lane and remains ahead of `origin/master`.
...
- Fresh deployed external browser audit is currently not green on the deployed `2297260` source because protected-route `_rsc` prefetches can hit 429 on mobile. Clean reviewed source `8d98f00` fixes this locally, but it still needs deploy and a rerun of the external audit before claiming deployed browser/search audit green.
- Still not public-launch ready: crawler indexing is intentionally disabled; public SEO opening needs explicit owner approval after final SEO/canonical checks. Fix Turnstile envs, deploy clean reviewed source `8d98f00`, then rerun deployed inquiry/browser smoke and the external browser audit before dealer outreach.

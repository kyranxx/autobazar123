...
- Local app-side Vercel preflight blockers were reduced, and local Vercel Preview packaging is now green:
  ...
  - Fresh 2026-06-21 continuation blocker refresh: `npm run check:live-rls-posture -- --json` still returned `ok=false` with 4 leaked probes (`profiles.email`, `profiles.phone`, `profiles.credit_balance`, and raw `dealers`), 0 probe errors, and 1 anon-readable row for each check. `npm run test:launch-migration-worktree-script` passed 7/7, and `npm run check:launch-migration-worktree` failed as expected in this dirty main worktree because `20260619214332_add_vehicle_taxonomy_metadata.sql` is present and the worktree has uncommitted launch-hardening changes. `git worktree list --porcelain` still shows only the main repo.
...
- Launch migration safety guard added: `npm run check:launch-migration-worktree -- --root <clean-worktree>` checks that a clean launch DB worktree has the required launch migrations, no dirty worktree entries, no dirty `supabase/migrations` entries, and no unrelated `20260619214332_add_vehicle_taxonomy_metadata.sql` unless explicitly overridden. The runbook now requires this before Supabase dry-runs or pushes.
  ...
  - Vercel env metadata-name gate now uses a pinned modern CLI path. `npm run test:vercel-env-names-script` passed 7/7. Real `npm run check:vercel-env-names` passed without pulling or printing secrets: Preview and Production each had 20/20 required runtime names present. This proves expected names exist in Vercel metadata, not that sensitive values are valid in cloud runtime.
  ...
  - Latest aggregate result is still blocked, but no longer by package-referenced local files, local performance, local Vercel Upstash env metadata/pull checks, local lint, or Vercel static-PPR packaging. Current blocker lanes are deploy-source readiness, live Supabase anon RLS with 4 leaked probes, and current-tree launch migration safety because the main worktree is dirty and the unrelated taxonomy metadata migration is present.
  ...
  - Fresh latest 2026-06-21 deploy-source readiness checker: root `.vercelignore` now excludes Supabase DB artifacts, launch-deferred taxonomy/provider operator scripts, deferred taxonomy helper source files, and local test/report output from Vercel source upload. `npm run test:deploy-source-readiness-script` passed 4/4. The current main worktree is not the deploy source because it is ahead of `origin/master` and still contains the deferred taxonomy/provider lane; use the clean reviewed worktree at `C:\Users\User\Desktop\Projects\ab123-rs-153336` commit `2129f7027d4fed0476075ff5add2dc7318ba65c5` unless a newer source is explicitly reverified.
  ...
  - Fresh latest 2026-06-21 `npm run check:launch-blockers -- --allow-extra-worktrees` exits 1 with three blocker lanes: deploy-source readiness, live Supabase anon RLS, and launch migration worktree safety. Passing lanes include branch/worktree cleanup (`master`, 1 worktree, `extraWorktreesAllowed=true`), package-script tracking with 111 local file paths, cleanup/refactor scan with 576 launch-code files, diff check, launch account/data coverage, security gate, dependency audit, payment suite 6 files / 52 tests, cron/email suite 10 files / 38 tests, fallback registry, Algolia/Supabase search parity 56/56, SEO/GEO 3 files / 31 tests, local smoke target coverage 4/4, image upload release suite 3 files / 18 tests, performance budget, lint, typecheck, production build, production bundle budget, Vercel env names/values, Vercel Upstash env value/metadata checks, and Vercel static-PPR packaging. No deploy, push, email send, payment action, or remote DB write was run.
...
- Older missing-secret compile/runtime blockers were reduced, but launch release is not green.
- Local production-mode performance is now verified: 6/6 Playwright audit tests, 80/80 route/viewport checks, 0 failing routes, 0 DevTools issues, and `npm run check:performance-budgets` passed with p95 JS transfer `2258037`, p95 main-thread work `363.1`, and p95 DOMContentLoaded `691.65`.
- Current verified status: Vercel Upstash local env gate is green under correct sensitive-variable semantics. Preview and Production have a non-empty pull-readable `UPSTASH_REDIS_REST_URL`, and `UPSTASH_REDIS_REST_TOKEN` exists as Vercel `type=sensitive` metadata. Cloud runtime/provider smoke is still required before trusting Preview/Production rate-limit behavior.
- Current verified status: local Vercel Preview packaging preflight is green. Fresh 2026-06-21 continuation `npm run check:vercel-build-preview` exited 0 with `Build completed successfully` for target `preview`, and follow-up `npm run check:vercel-ppr-lambda-blocker` reports 0 partially-static routes with no `next-resume` PPR chain headers. Preview deployment itself was not run.
- Current verified blocker: live Supabase anon reads still expose `profiles.email`, `profiles.phone`, `profiles.credit_balance`, and raw `dealers`; the fresh 2026-06-21 continuation run of `npm run check:live-rls-posture -- --json` returned `ok=false`, 4 leaked probes, 0 probe errors, and 1 anon-readable row for each check. Use the same command again after compatible code is deployed and the launch migrations are applied.
- Current safety requirement: before any remote Supabase dry-run or push, create a clean launch worktree and run `npm run check:launch-migration-worktree -- --root <clean-worktree>` so the unrelated taxonomy metadata migration cannot be pushed by accident. Fresh 2026-06-21 continuation evidence: the guard tests passed 7/7 and the guard failed as expected in this dirty main worktree because the taxonomy metadata migration is present.
- Current approval requirement: `docs/launch-remote-migration-deploy-runbook.md` now has a short owner approval packet. It requires separate explicit owner approval before Preview deploy, Production deploy, the exact remote migration apply command, and enabling Production indexing/public SEO.
- Use `npm run check:launch-blockers` for the shortest current blocker rollup. The latest short rollup exits non-zero with three blocker lanes: deploy-source readiness, live Supabase anon RLS, and current-tree migration safety. Package-script tracking now passes with 111 package-referenced local file paths after fixing adjacent-path detection, staging approved launch-support files, removing deferred taxonomy/provider package shortcuts, and adding the deploy-source checker scripts. Use `npm run check:launch-blockers:full` for the stronger local predeploy rollup that also includes the Vercel Preview build preflight; if it runs from an explicit clean review-source worktree while the main worktree still exists, append `-- --allow-extra-worktrees`.
- Fresh 2026-06-21 deploy-source checker integration recheck: `npm run check:launch-blockers -- --allow-extra-worktrees` still exits 1, now on deploy-source readiness, live Supabase anon RLS, and current-tree launch migration safety. Passing lanes include branch/worktree cleanup with the explicit review-worktree mode, package-script tracking with 111 paths, cleanup/refactor scan with 576 files, diff check, launch account/data coverage, security, dependencies, payments 6 files / 52 tests, cron/email 10 files / 38 tests, fallbacks, Algolia/Supabase 56/56 parity, SEO/GEO 3 files / 31 tests, local smoke target coverage 4/4, uploads 3 files / 18 tests, performance, lint, typecheck, production build, production bundle budget, Vercel env metadata/value checks, and Vercel static-PPR packaging. No deploy, push, email send, payment action, or remote DB write was run.
- Fresh earlier 2026-06-21 full predeploy recheck before deploy-source readiness was integrated: `npm run check:launch-blockers:full` exited 1 only on live Supabase anon RLS and current-tree launch migration safety. The full-only Vercel Preview local build preflight passed with `vercel@54.14.5` and detected Next.js `16.2.9`; local build/test/service gates above also passed. Rerun the full rollup from the chosen reviewed deploy source before Preview approval.
- Fresh 2026-06-21 deploy-source readiness checker: Vercel CLI deploys from the project root/source path and uploads project files unless default-excluded or excluded by `.vercelignore`; this repo now has a root `.vercelignore` with launch-risk exclusions for Supabase DB artifacts, deferred taxonomy/provider operator scripts, deferred taxonomy helper source files, and local test/report output. A plain `npx vercel@54.14.5 deploy` from the current main worktree would not be a clean reviewed-source deploy because main is ahead of `origin/master` and still contains the deferred taxonomy/provider lane. Preferred next step is Preview deploy approval from clean reviewed source `C:\Users\User\Desktop\Projects\ab123-rs-153336` at `2129f7027d4fed0476075ff5add2dc7318ba65c5`, unless owner explicitly accepts a dirty current-worktree deploy.
- Fresh latest 2026-06-21 reviewed-source verification: clean detached worktree `C:\Users\User\Desktop\Projects\ab123-rs-153336` is now at local commit `2129f7027d4fed0476075ff5add2dc7318ba65c5` (`2129f70`) and is the current reviewed deploy source. The only diff from the previously audited `d38f2f3a5e27164ac759bcbe1e8a9fb6d36e7c6d` source is documentation/status files, so the prior production-mode Chrome audit evidence still applies to the same runtime tree: 6/6 tests, 80/80 route/viewport checks, 0 failing routes, 0 DevTools issues, and `npm run check:performance-budgets` p95 JS transfer `1639621`, p95 main-thread work `306.2`, p95 DOMContentLoaded `545.25`. Fresh checks at `2129f7027d4fed0476075ff5add2dc7318ba65c5` passed `npm run check:deploy-source-readiness`, `npm run check:launch-migration-worktree`, and `npm run test:launch-blockers-script` 18/18; direct path checks confirm the deferred taxonomy/provider scripts/helpers plus `20260619214332_add_vehicle_taxonomy_metadata.sql` are absent. Fresh `npm run check:launch-blockers:full -- --allow-extra-worktrees` from the reviewed source exits 1 only on live Supabase anon RLS; branch/worktree cleanup, deploy-source readiness, package tracking, cleanup scan, diff check, launch account/data coverage, security, dependency audit, payments, cron/email, fallbacks, Algolia parity, SEO/GEO, local smoke coverage, uploads, performance, lint, typecheck, production build, bundle budget, Vercel env checks, static-PPR packaging, Vercel Preview build preflight, and launch migration worktree safety all pass. No deploy, push, email send, payment action, or remote DB write was run.
- Fresh latest 2026-06-21 clean reviewed-source Supabase dry-run: Supabase CLI still reports `2.107.0`, and `npx supabase db push --help` still has `--dry-run` and `--include-all` but no per-file migration selector. The reviewed worktree `C:\Users\User\Desktop\Projects\ab123-rs-153336` remains linked locally to non-secret project ref `vxwbbzjlctjpzivfkdou`; link metadata lives in ignored `supabase/.temp`. `npm run check:launch-migration-worktree` passed there. `npx supabase migration list` showed exactly `20260618174500_harden_profile_dealer_public_reads.sql`, `20260618193000_align_payment_notifications_billing.sql`, and `20260620010000_harden_billing_checkout_atomicity.sql` as local-only; `20260619120000_add_vehicle_taxonomy_candidates.sql` is present locally and remotely; the blocked taxonomy metadata migration is absent. `npx supabase db push --dry-run --include-all` would push only those three launch-critical migrations and did not apply anything.
- Final 2026-06-21 pre-approval refresh from the reviewed source reran `npm run check:launch-blockers:full -- --allow-extra-worktrees`. It exited 1 only because `npm run check:live-rls-posture -- --json` still reports 4 live anon RLS leaks. The same run passed branch/worktree cleanup, Vercel deploy-source readiness, package-script tracking, cleanup/refactor markers, `git diff --check`, launch account/data coverage, security release gate, `npm audit`, payment release tests, cron/email release tests, fallback registry, Algolia/Supabase parity, SEO/GEO, local smoke coverage, uploads, performance budgets, lint/static UI guards, typecheck, production build, bundle budget, Vercel env metadata/value checks, static-PPR checker, Vercel Preview local build preflight, and launch migration worktree safety. No deploy, push, remote DB write, email send, or payment action was run. There is no remaining useful non-mutating launch gate before explicit Preview deploy approval.
- Preview/production env names exist by metadata checks, but sensitive deployed runtime values still need cloud build/runtime smoke after deploy approval.
...
- Crawler blocking is verified live: `/robots.txt` returns `User-Agent: *` plus `Disallow: /`, homepage has `X-Robots-Tag: noindex, nofollow, noarchive`, and HTML meta robots is `noindex, nofollow`. A fresh Chrome/Playwright read-only check found 0 console warnings/errors and 0 page errors on the live homepage. Live canonical/sitemap/llms output still uses apex `autobazar123.sk`, proving the local `www` SEO fixes are not deployed yet.
...
1. Ask for explicit Preview deploy approval from reviewed source `C:\Users\User\Desktop\Projects\ab123-rs-153336` at `2129f7027d4fed0476075ff5add2dc7318ba65c5`, then run Preview deploy and smoke if approved.
2. If Preview is green, get explicit Production compatible-code deploy approval, deploy with indexing still disabled, smoke Production, then request exact remote Supabase migration approval from the clean migration worktree/runbook path.
3. After migrations, rerun live RLS, payment notification logging, preview/production health, maintenance bypass, cron/email, and core auth/listing/payment smoke before public SEO or dealer outreach.

...
- Fresh read-only Production external browser audit found a deployed mobile prefetch/rate-limit issue: `TEST_URL=https://www.autobazar123.sk PLAYWRIGHT_CHROMIUM_CHANNEL=chrome WEBAPP_AUDIT_MODE=external AUDIT_MAX_ROUTES=20 npm run audit:webapp` exited 1 with 7/40 failing route/viewport checks. All failing checks were mobile pages whose visible protected navigation links triggered `_rsc` requests to `/moj-ucet`, `/dealer`, or `/moj-ucet?tab=create` and received 429; page status stayed 200 and DevTools issues stayed 0.
- Clean reviewed source `C:\Users\User\Desktop\Projects\ab123-rs-153336` is now advanced locally to `68782c2`, including the SEO title/H1 fix and the `_rsc` prefetch fix. The full reviewed-source rollup `npm run check:launch-blockers:full -- --allow-extra-worktrees` is blocked only by missing Turnstile Vercel env metadata names; all other lanes passed, including deploy-source readiness, migration safety, security, payments, cron/email, Algolia, SEO/GEO, uploads, performance, lint, typecheck, build, Vercel Preview build preflight, Upstash env metadata/value checks, and live RLS. The reviewed worktree stayed clean.
- Current main and clean reviewed source `68782c2` have a local root-cause fix for the deployed prefetch noise: Next `_rsc` prefetch requests on protected routes no longer consume the protected-route rate-limit budget before redirect/RBAC handling. RED/GREEN evidence: new proxy regression first failed because `checkRateLimit` was called once, then passed after the fix; `npx vitest run src/proxy.test.ts` now passes 19/19. Support checks passed: `npm run test:security:release-gate`, `npm run typecheck`, `npm run lint`, and `git diff --check`. Not deployed yet.
- Still not public-launch ready: after Turnstile keys are configured, deploy clean reviewed source `68782c2`, then rerun deployed inquiry/browser smoke and the external browser audit before dealer outreach.

## 2026-06-21 post-deploy/migration/payment update

- Last deployed app source was `C:\Users\User\Desktop\Projects\ab123-rs-153336` at `2297260` (`fix: handle stripe failed payment intents`). The same clean detached reviewed worktree has now been advanced locally to `68782c2` (`fix: skip rate limit for rsc protected prefetches`) for the next reviewed-source checks; it has not been pushed or deployed. The main repo still has the separate dirty taxonomy lane and remains ahead of `origin/master`.
...
- Fresh latest reviewed-source full rollup from `C:\Users\User\Desktop\Projects\ab123-rs-153336` at `68782c2` ran `npm run check:launch-blockers:full -- --allow-extra-worktrees`. It is blocked only by missing Turnstile Vercel env metadata names. Passing lanes include branch/worktree state, deploy-source readiness, package-script tracking, cleanup/refactor scan, `git diff --check`, launch account coverage, security release gate, `npm audit`, payments 6 files / 53 tests, cron/email 10 files / 38 tests, fallbacks, Algolia parity 56/56, SEO/GEO 3 files / 32 tests, local smoke helper 4/4, uploads 3 files / 19 tests, performance budgets, lint, typecheck, build, production bundle budget, Upstash env checks, live RLS, PPR packaging, Vercel Preview local build preflight, and launch migration worktree safety. The reviewed worktree remained clean after the run.
- Fresh latest reviewed-source UI quality gate from `8a6f520` passed with installed Chrome: `PLAYWRIGHT_CHROMIUM_CHANNEL=chrome npm run test:ui-quality-gate` ran 18 Playwright web-interface checks across desktop, Pixel 7, and iPhone 13 landscape plus 19 UI unit tests, and ended with `UI QUALITY GATE: OK`. The reviewed worktree remained clean after the run.
- Fresh latest reviewed-source accessibility gates from `8a6f520` passed with installed Chrome: `PLAYWRIGHT_CHROMIUM_CHANNEL=chrome npm run test:a11y` passed 63/63, `npm run test:keyboard` passed 9/9, `npm run test:sr-proxy` passed 42/42, and `npm run test:mobile-matrix` passed 42/42. The reviewed worktree remained clean after these runs.
...
- Fresh current-main full non-mutating rollup `npm run check:launch-blockers:full -- --allow-extra-worktrees` has the same three expected current-main blockers and additionally passes the pinned Vercel Preview local build preflight with `vercel@54.14.5`. The latest clean reviewed-source full rollup at `68782c2` narrows this to the single external Turnstile env blocker, and the latest clean-source UI/accessibility gates pass.
...
- Fresh deployed external browser audit is currently not green on the deployed `2297260` source because protected-route `_rsc` prefetches can hit 429 on mobile. Clean reviewed source `68782c2` fixes this locally, but it still needs deploy and a rerun of the external audit before claiming deployed browser/search audit green.
- Still not public-launch ready: crawler indexing is intentionally disabled; public SEO opening needs explicit owner approval after final SEO/canonical checks. Fix Turnstile envs, deploy clean reviewed source `68782c2`, then rerun deployed inquiry/browser smoke and the external browser audit before dealer outreach.

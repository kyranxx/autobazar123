...
- Fresh current-main short launch rollup after adding the explicit upload smoke command: `npm run check:launch-blockers -- --allow-extra-worktrees` remains blocked only by the expected current-main posture: dirty deploy source, missing Turnstile Vercel env metadata names, and current-tree launch migration safety because the separate taxonomy metadata migration is still present. Passing lanes include package-script tracking for 113 local paths, cleanup/refactor scan across 580 launch-code files, security release gate, `npm audit`, payments 53/53, cron/email 38/38, Algolia parity 56/56, SEO/GEO 32/32, upload release 19/19, performance, lint, typecheck, build, production bundle budget, Upstash env checks, live RLS, and Vercel static-PPR packaging.

...
- Fresh Chrome Turnstile dashboard attempt reached the Cloudflare Turnstile account page, but Chrome automation was blocked by an open extension popup before any widget/env change could be made. The Turnstile tab was left open as a handoff. No Cloudflare or Vercel settings were changed.
...
- Fresh real Production Cloudflare Images upload smoke passed with cleanup: `TEST_URL=https://www.autobazar123.sk PLAYWRIGHT_CHROMIUM_CHANNEL=chrome npm run smoke:cloudflare-upload` logged in with the configured seller account, requested `/api/images/upload-url`, uploaded `public/placeholder-car.jpg` through Cloudflare direct upload, received an `imagedelivery.net` public variant, and deleted the uploaded Cloudflare image. Safe helper verification passed first: `node --check scripts/smoke-cloudflare-image-upload.mjs`, `npm run test:cloudflare-upload-smoke-script` 5/5, and `git diff --check`.
- Fresh local real Stripe dealer top-up checkout creation smoke passed with cleanup: `PLAYWRIGHT_CHROMIUM_CHANNEL=chrome npm run smoke:stripe-dealer-topup` logged in with the configured dealer account on `localhost`, created a test-mode `dealer_100` Checkout Session, verified the `billing_checkout_sessions` row, expired the Stripe session, deleted 1 checkout row and 1 idempotency row, left 0 webhook-log residue, and proved dealer prepaid balance stayed unchanged. Helper verification passed first: `node --check scripts/smoke-stripe-dealer-topup.mjs`, `npm run test:stripe-dealer-topup-smoke-script` 8/8, `npm run test:payments-release` 6 files / 53 tests, and `npm run test:security:release-gate`.
- Fresh protected Preview real Stripe dealer top-up checkout creation smoke passed with cleanup through a temporary Vercel protected-preview access URL: `TEST_URL=<protected-preview-share-url> PLAYWRIGHT_CHROMIUM_CHANNEL=chrome npm run smoke:stripe-dealer-topup` logged in with the configured dealer account on Preview, created a test-mode `dealer_100` Checkout Session, verified the `billing_checkout_sessions` row, expired the Stripe session, deleted 1 checkout row, 1 idempotency row, and 1 webhook log row, and proved dealer prepaid balance stayed unchanged. The share token was not stored. Support checks: `node --check scripts/smoke-stripe-dealer-topup.mjs`, `npm run test:stripe-dealer-topup-smoke-script` 8/8, `npm run test:payments-release` 6 files / 53 tests, and `npm run test:security:release-gate`.
- Fresh current-main short launch rollup after adding the explicit dealer top-up smoke command: `npm run check:launch-blockers -- --allow-extra-worktrees` remains blocked only by the expected current-main posture: dirty deploy source from the separate taxonomy lane, missing Turnstile Vercel env metadata names, and current-tree launch migration safety because the separate taxonomy metadata migration is still present. Passing lanes include package-script tracking for 115 local paths, cleanup/refactor scan across 582 launch-code files, security release gate, `npm audit`, payments 53/53, cron/email 38/38, Algolia parity 56/56, SEO/GEO 32/32, upload release 19/19, performance, lint, typecheck, build, production bundle budget, Upstash env checks, live RLS, and Vercel static-PPR packaging.
- Fresh dirty taxonomy/discovery lane recheck passed focused tests and support checks without staging or mutation: 7 taxonomy Vitest files / 18 tests, `npm run typecheck`, `npm run lint`, and `git diff --check`. No-mode discovery/promotion scripts fail closed before mutation. Read-only checks: promotion `--dry-run` reported 0 approved candidates / 0 brand inserts / 0 model inserts, Wikidata `--dry-run --limit=1` returned 0 candidates, Autobazar.eu Tesla `--dry-run` returned 4 model candidates, and Otomoto `--dry-run` returned 115 candidates. Current-main `npm run check:deploy-source-readiness` and `npm run check:launch-migration-worktree -- --root C:\Users\User\Desktop\Projects\autobazar123` still block this lane from deploy/DB push because the dirty taxonomy files and local-only `20260619214332_add_vehicle_taxonomy_metadata.sql` are present.
...
- Clean reviewed source `C:\Users\User\Desktop\Projects\ab123-rs-153336` is now advanced locally to `d4bdccf`, carrying the SEO title/H1 fix, `_rsc` prefetch fix, Cloudflare upload smoke helper, dealer top-up smoke helper, and fresh Turnstile blocker/runbook docs. Fresh guards passed there: `git diff --check`, `npm run check:deploy-source-readiness`, `npm run check:launch-migration-worktree -- --root C:\Users\User\Desktop\Projects\ab123-rs-153336`, and `npm run check:launch-blockers:full -- --allow-extra-worktrees` with only the expected missing Turnstile Vercel env metadata blocker. Passing full-rollup lanes included package-script tracking for 115 paths, cleanup/refactor scan across 563 launch-code files, security release gate, `npm audit`, payments 53/53, cron/email 38/38, Algolia 56/56, SEO/GEO 32/32, uploads 19/19, performance, lint, typecheck, build, production bundle budget, Upstash env checks, live RLS, Vercel static-PPR packaging, Vercel Preview local build preflight, and launch migration worktree safety. The reviewed worktree stayed clean.
- Current main and clean reviewed source `d4bdccf` have a local root-cause fix for the deployed prefetch noise: Next `_rsc` prefetch requests on protected routes no longer consume the protected-route rate-limit budget before redirect/RBAC handling. RED/GREEN evidence from the earlier `68782c2` fix remains: new proxy regression first failed because `checkRateLimit` was called once, then passed after the fix; `npx vitest run src/proxy.test.ts` passes 19/19 inside the fresh full rollup. Not deployed yet.
- Still not public-launch ready: after Turnstile keys are configured, deploy clean reviewed source `d4bdccf`, then rerun deployed inquiry/browser smoke and the external browser audit before dealer outreach.

## 2026-06-21 post-deploy/migration/payment update

- Last deployed app source was `C:\Users\User\Desktop\Projects\ab123-rs-153336` at `2297260` (`fix: handle stripe failed payment intents`). The same clean detached reviewed worktree has now been advanced locally to `d4bdccf` (`docs: record clean reviewed source rollup`) for the next reviewed-source checks; it has not been pushed or deployed. The main repo still has the separate dirty taxonomy lane and remains ahead of `origin/master`.
...
- Fresh deployed external browser audit is currently not green on the deployed `2297260` source because protected-route `_rsc` prefetches can hit 429 on mobile. Clean reviewed source `d4bdccf` fixes this locally, but it still needs deploy and a rerun of the external audit before claiming deployed browser/search audit green.
- Still not public-launch ready: crawler indexing is intentionally disabled; public SEO opening needs explicit owner approval after final SEO/canonical checks. Fix Turnstile envs, deploy clean reviewed source `d4bdccf`, then rerun deployed inquiry/browser smoke and the external browser audit before dealer outreach.
...
- Dirty taxonomy/discovery lane rechecked locally and remains separate from launch-critical DB work:
  ...
  - No-mode smoke checks for Wikidata, Autobazar.eu, Otomoto, Mobile.de, and promotion exit before mutation with explicit mode-required errors.
  - Read-only smoke checks passed: `npx tsx scripts/promote-approved-taxonomy-candidates.ts --dry-run` reported 0 approved candidates, 0 brand inserts, and 0 model inserts; `npx tsx scripts/discover-wikidata-taxonomy.ts --dry-run --limit=1` returned 0 candidates; `npx tsx scripts/discover-autobazar-eu-taxonomy.ts --dry-run --brands=tesla:Tesla` returned 4 model candidates; `npx tsx scripts/discover-otomoto-taxonomy.ts --dry-run` returned 115 candidates.
  - Passed: `git diff --check`, `npm run typecheck`, and `npm run lint`.
  - Current-main launch source guards still block this lane as intended: `npm run check:deploy-source-readiness` blocks on dirty deploy source/deferred taxonomy source, and `npm run check:launch-migration-worktree -- --root C:\Users\User\Desktop\Projects\autobazar123` blocks on dirty worktree plus local-only taxonomy metadata migration.
...
- Dirty taxonomy/discovery lane was audited but is not part of the launch-critical push:
  - Current worktree still has unrelated taxonomy changes in `src/lib/vehicle-taxonomy/public.ts`, new discovery scripts, parser tests, candidate-store helpers, and `supabase/migrations/20260619214332_add_vehicle_taxonomy_metadata.sql`.

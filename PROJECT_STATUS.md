...
- The Vercel env-name gate now requires Turnstile envs. `npm run test:vercel-env-names-script` passes 8/8, and real `npm run check:vercel-env-names` now blocks as expected on missing `NEXT_PUBLIC_TURNSTILE_SITE_KEY` and `TURNSTILE_SECRET_KEY` in both Preview and Production without printing secret values.

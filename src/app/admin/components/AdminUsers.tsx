...
import { getAdminUsers, updateUserCredits, banUser, type AdminUser } from '../actions'
    ...
    <tr className="border-b border-border-subtle hover:bg-surface-hover transition-colors">
      <td className="py-4 px-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-text-secondary">{userData.full_name || '-'}</p>
          </div>
        </div>
      </td>
      <td className="py-4 px-4">
      ...
      <td className="py-4 px-4">
        <div className="flex items-center gap-2">
          <span className="font-semibold text-text-primary">{userData.credit_balance}</span>
          <button
            onClick={onEditCredits}
            className="p-1 rounded hover:bg-background-tertiary transition-colors"
          >
            <svg className="w-4 h-4 text-text-secondary hover:text-accent" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export function AdminUsers() {
  ...
  const [editModal, setEditModal] = useState<{ open: boolean; user: AdminUser | null }>({
    open: false,
    user: null,
  })
  const [banModal, setBanModal] = useState<{ open: boolean; user: AdminUser | null }>({
    open: false,
    user: null,
  })
  ...
  const handleSaveCredits = async (newCredits: number) => {
    ...
    try {
      ...
      setUsers((prev) =>
        prev.map((u) =>
          u.id === editModal.user?.id ? { ...u, credit_balance: newCredits } : u
        )
      )
  ...
  const handleBanUser = async (reason: string) => {
    ...
    try {
      await banUser(banModal.user.id, currentUser.id, reason)
      setUsers((prev) =>
        prev.map((u) =>
              ...
              {loading ? (
                ...
                users.map((userData) => (
                  <UserRow
                    key={userData.id}
                    user={userData}
                    onEditCredits={() => setEditModal({ open: true, user: userData })}

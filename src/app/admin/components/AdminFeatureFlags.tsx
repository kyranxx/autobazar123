...
import { getFeatureFlags, toggleFeatureFlag, createFeatureFlag, type FeatureFlag } from '../actions'
    ...
    <div className="flex items-center justify-between p-4 border-b border-border-subtle last:border-0 hover:bg-surface-hover transition-colors">
      <div className="flex-1">
        <div className="flex items-center gap-3 mb-1">
          <h4 className="font-medium text-text-primary font-mono">{flag.key}</h4>
...
function CreateFlagModal({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      try {
        await onCreate(key.trim().toLowerCase().replace(/\s+/g, '_'), description.trim())
        setKey('')
        setDescription('')
        onClose()
      } catch (error) {
...
const DEFAULT_FLAGS: Omit<FeatureFlag, 'id'>[] = [
  { key: 'enable_stripe_payments', enabled: true, description: 'Povoliť Stripe platby', created_at: '', updated_at: '' },
  { key: 'enable_dealer_features', enabled: true, description: 'Povoliť funkcie pre dealerov', created_at: '', updated_at: '' },
  { key: 'enable_photo_upload', enabled: true, description: 'Povoliť nahrávanie fotografií', created_at: '', updated_at: '' },
  { key: 'enable_notifications', enabled: false, description: 'Povoliť push notifikácie', created_at: '', updated_at: '' },
  { key: 'enable_advanced_search', enabled: true, description: 'Povoliť pokročilé vyhľadávanie', created_at: '', updated_at: '' },
  { key: 'maintenance_mode_soft', enabled: false, description: 'Jemný údržbový režim (banner)', created_at: '', updated_at: '' },
]
...
export function AdminFeatureFlags() {
  ...
  const handleToggle = async (flag: FeatureFlag) => {
    ...
    try {
      if (flag.id.startsWith('default_')) {
        ...
        await toggleFeatureFlag(flag.id, !flag.enabled, user.id)
        setFlags((prev) =>
          prev.map((f) =>
            f.id === flag.id ? { ...f, enabled: !f.enabled, updated_at: new Date().toISOString() } : f
          )
  ...
  const handleCreate = async (key: string, description: string) => {
    ...
    try {
      await createFeatureFlag(key, description, user.id)
    ...
    return (
      <Card>
        ...
        <CardContent className="space-y-4">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="flex items-center justify-between p-4 border-b border-border-subtle">
        ...
        <CardContent className="p-0">
          {flags.length === 0 ? (
            <div className="py-12 text-center text-text-secondary">
              ...
              <Button variant="accent" size="sm" className="mt-4" onClick={() => setCreateModalOpen(true)}>
                Vytvoriť prvý flag
              </Button>
            </div>
          ) : (
        ...
        <CardContent className="p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="text-sm text-text-secondary">
              <p className="font-medium text-text-primary mb-1">Tip: Použitie feature flagov</p>
              <p>
                V kóde použite <code className="px-1.5 py-0.5 rounded bg-background-secondary font-mono text-xs">featureFlags.isEnabled(&apos;key&apos;)</code> 
                na kontrolu stavu. Zmeny sa prejavia okamžite bez potreby reštartu.
              </p>
            </div>
          </div>
        </CardContent>

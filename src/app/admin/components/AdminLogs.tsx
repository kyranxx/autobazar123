...
import { getSystemLogs, getAuditLogs, type SystemLog, type AuditLog } from '../actions'

function LogLevelBadge({ level }: { level: string }) {
  const variants: Record<string, 'default' | 'success' | 'warning' | 'error' | 'accent'> = {
    debug: 'default',
    info: 'success',
    warn: 'warning',
    error: 'error',
...
function CategoryBadge({ category }: { category: string }) {
  ...
  return (
    <span className={`px-2 py-0.5 rounded text-xs font-medium ${colors[category] || colors.system}`}>
      {category}
    </span>
  )
}

function SystemLogRow({ log, onClick }: { log: SystemLog; onClick: () => void }) {
  return (
    <tr 
...
function AuditLogRow({ log, onClick }: { log: AuditLog; onClick: () => void }) {
  ...
  const getActionIcon = (action: string) => {
    if (action.includes('approve') || action.includes('unban')) {
      return <svg className="w-4 h-4 text-success" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
    }
    if (action.includes('reject') || action.includes('delete') || action.includes('ban')) {
      return <svg className="w-4 h-4 text-error" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
    }
    if (action.includes('update') || action.includes('edit')) {
      return <svg className="w-4 h-4 text-accent" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
    }
    return <svg className="w-4 h-4 text-text-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
  }
    ...
    >
      ...
      <td className="py-3 px-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-text-primary">{getActionLabel(log.action)}</span>
        </div>
      </td>
      <td className="py-3 px-4">
        <span className="text-text-secondary">{log.admin_email || log.admin_id.substring(0, 8)}</span>
      </td>
      <td className="py-3 px-4">
        <Badge variant="default" size="sm">{log.target_type}</Badge>
      </td>
      <td className="py-3 px-4">
        {log.target_id && (
          <span className="text-sm text-text-muted font-mono">
  ...
  return (
    <Modal open={open} onClose={onClose} title="Detail záznamu" size="lg">
      <div className="space-y-4">
            ...
            <>
              ...
              {(log as SystemLog).request_id && (
                <div>
                  ...
                  <p className="font-mono text-sm text-text-muted">{(log as SystemLog).request_id}</p>
                </div>
              )}
            </>
          )}
          {!isSystemLog && (
            <>
              ...
              <div>
                ...
                <p className="text-text-primary">{(log as AuditLog).admin_email || (log as AuditLog).admin_id}</p>
              </div>
              <div>
                <p className="text-sm text-text-secondary mb-1">Cieľ</p>
                <p className="text-text-primary">{(log as AuditLog).target_type}: {(log as AuditLog).target_id}</p>
              </div>
            </>
          )}
        </div>
        ...
        {((isSystemLog && (log as SystemLog).metadata) || (!isSystemLog && (log as AuditLog).details)) && (
          <div>
            ...
            <pre className="text-sm text-text-muted bg-background-tertiary p-3 rounded-lg overflow-auto max-h-48">
              {JSON.stringify(isSystemLog ? (log as SystemLog).metadata : (log as AuditLog).details, null, 2)}
            </pre>
          </div>
        )}
...
function LogFilters({
  ...
  return (
    <div className="flex flex-wrap gap-4">
      ...
      <div>
        <label className="block text-sm text-text-secondary mb-1">Kategória</label>
        <select
          value={category}
...
export function AdminLogs() {
  ...
  const handleRefresh = async () => {
    ...
    try {
      if (activeTab === 'system') {
        const logs = await getSystemLogs(levelFilter || undefined, categoryFilter || undefined)
        setSystemLogs(logs)
      } else {
        const logs = await getAuditLogs()
        setAuditLogs(logs)
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-border-subtle bg-background-tertiary">
                      <th className="py-3 px-4 text-left text-sm font-medium text-text-secondary">Čas</th>
                      <th className="py-3 px-4 text-left text-sm font-medium text-text-secondary">Úroveň</th>
                      <th className="py-3 px-4 text-left text-sm font-medium text-text-secondary">Kategória</th>
                      <th className="py-3 px-4 text-left text-sm font-medium text-text-secondary">Správa</th>
                      <th className="py-3 px-4 text-left text-sm font-medium text-text-secondary">User ID</th>
                    </tr>
                  </thead>
                  <tbody>
                    {loading ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-border-subtle bg-background-tertiary">
                      <th className="py-3 px-4 text-left text-sm font-medium text-text-secondary">Čas</th>
                      <th className="py-3 px-4 text-left text-sm font-medium text-text-secondary">Akcia</th>
                      <th className="py-3 px-4 text-left text-sm font-medium text-text-secondary">Admin</th>
                      <th className="py-3 px-4 text-left text-sm font-medium text-text-secondary">Typ</th>
                      <th className="py-3 px-4 text-left text-sm font-medium text-text-secondary">Cieľ</th>
                    </tr>
                  </thead>
                  <tbody>
                    {loading ? (

...
function getMarkSoldRateLimitIdentifier(request: NextRequest): string {
  return createRateLimitIdentifier("account_ads_mark_sold", request.headers);
}
...
export async function POST(request: NextRequest) {
  ...
  const rate = await checkStrictRateLimit(getMarkSoldRateLimitIdentifier(request));
  if (!rate.success) {

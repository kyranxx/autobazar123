    ...
    try {
        ...
        if (topFetchError) {
            ...
            const { error: topUpdateError } = await supabaseAdmin
                .from("ads")
                .update({ is_top_ad: false, top_ad_expires_at: null, updated_at: now })
        ...
        if (highlightFetchError) {
            ...
            const { error: highlightUpdateError } = await supabaseAdmin
                .from("ads")
                .update({ is_highlighted: false, highlighted_expires_at: null, updated_at: now })

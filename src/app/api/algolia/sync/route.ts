...
import { getAdminClient, CARS_INDEX, transformCarToAlgoliaRecord } from "@/lib/algolia";
...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        const records = (ads as unknown as SupabaseAd[]).map(transformCarToAlgoliaRecord);
        ...
        return NextResponse.json(
            { error: "Failed to sync to Algolia", details: String(error) },
            { status: 500 }
        );
    }
}
...
export async function DELETE(request: NextRequest) {
    ...
    try {
        ...
        return NextResponse.json(
            { error: "Failed to clear Algolia index", details: String(error) },
            { status: 500 }
        );
    }
}

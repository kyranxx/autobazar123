...
export function FilterSidebar() {
    ...
    const { canRefine: hasActiveFilters, refine: clearAll } = useClearRefinements();

    return (
        <div className="space-y-1">
            {/* Active Filters */}
            <ActiveFiltersSection hasActiveFilters={hasActiveFilters} clearAll={clearAll} t={t} />
        ...
        <div className="pb-4 mb-4 border-b border-border-subtle">
            ...
            <CurrentRefinements
                classNames={{
                    ...
                    item: "contents",
                    label: "hidden",
                    category: "inline-flex items-center gap-1.5 px-2.5 py-1.5 bg-accent/10 border border-accent/20 rounded-lg text-xs font-medium text-accent",
...
const refinementListClasses = {
    ...
    checkbox: "w-4 h-4 rounded border-2 border-border-strong text-accent focus:ring-accent focus:ring-offset-0 transition-colors",
    labelText: "text-sm text-text-secondary group-hover:text-text-primary transition-colors truncate flex-1",
        ...
        <div className="space-y-3">
            <RangeInput
                ...
                classNames={{
                    ...
                    input: "w-full px-3 py-2.5 bg-background border border-border-subtle rounded-lg text-sm text-text-primary placeholder:text-text-muted focus:border-accent focus:ring-1 focus:ring-accent/30 transition-all",
                    separator: "text-text-muted font-medium",
                    submit: "px-4 py-2.5 bg-accent text-white rounded-lg text-sm font-semibold hover:bg-accent-hover transition-colors shadow-sm",
                }}
...
function AllBrandsRefinementList() {
    ...
    const mergedItems = useMemo(() => {
        ...
        return allBrands
            .map(b => ({ value: b.value, label: b.value, isRefined: refined.has(b.value), count: b.count }))

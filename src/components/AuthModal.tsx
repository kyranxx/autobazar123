...
export default function AuthModal({ isOpen, onClose, initialView = "login" }: AuthModalProps) {
    ...
    return (
        ...
        >
            <div
                ref={modalRef}
                className="relative w-full max-w-md bg-background-secondary rounded-2xl shadow-xl overflow-hidden animate-in fade-in zoom-in-95 duration-200"
            >
                {/* Close Button */}
                <button
                    onClick={onClose}
                    ...
                    {view === "register" && (
                        <form onSubmit={handleRegister} className="space-y-4">
                            ...
                            <label className="flex items-start gap-2 text-sm text-text-secondary cursor-pointer">
                                ...
                                <span>
                                    Súhlasím s{" "}
                                    <a href="/obchodne-podmienky" className="text-accent hover:underline" target="_blank">
                                        podmienkami
                                    </a>{" "}
                                    a{" "}
                                    <a href="/ochrana-udajov" className="text-accent hover:underline" target="_blank">
                                        ochranou údajov
                                    </a>
                                </span>
                            </label>
                ...
                <div className="px-6 py-4 bg-background-tertiary/50 border-t border-border-subtle text-center">
                    {view === "login" ? (
                        <p className="text-sm text-text-secondary">
                            Nemáte účet?{" "}
                            <button onClick={() => setView("register")} className="text-accent font-medium hover:underline">
                                Registrujte sa
                            </button>
                        </p>
                    ) : view === "register" ? (
                        <p className="text-sm text-text-secondary">
                            Už máte účet?{" "}
                            <button onClick={() => setView("login")} className="text-accent font-medium hover:underline">
                                Prihláste sa
                            </button>
                        </p>
                    ) : null}

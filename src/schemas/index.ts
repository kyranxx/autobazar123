...
export const AdBasicInfoSchema = z.object({
    ...
    brand: z.string().min(1, 'Značka je povinná').max(50, 'Názov značky je príliš dlhý'),
    model: z.string().min(1, 'Model je povinný').max(50, 'Názov modelu je príliš dlhý'),
...
export const AdLocationSchema = z.object({
    ...
    location_district: z.string().max(100, 'Názov okresu je príliš dlhý').optional(),
    description: z
        .string()
...
export const AdSearchFiltersSchema = z.object({
    ...
    year_to: z.number().int().max(currentYear + 1).optional(),
...
export type CreditTransactionAction = z.infer<typeof CreditTransactionActionSchema>;

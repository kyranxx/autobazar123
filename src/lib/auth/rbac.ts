 ...
 */
export async function getUserRole(userId: string | null): Promise<UserRoleInfo> {
    if (!userId) {
    ...
    if (!serviceRoleKey) {
        console.warn('SUPABASE_SERVICE_ROLE_KEY not set, falling back to user role');
...
export async function requireRole(
    ...
    if (roleHierarchy[userRole.role] < roleHierarchy[requiredRole]) {
        throw new Error(`Role ${requiredRole} required, but user has role ${userRole.role}`);
    }

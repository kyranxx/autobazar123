...
import { DEFAULT_FLAGS, FeatureFlag, FeatureFlagKey } from '@/config/feature-flags';
...
function isUserTargeted(userId: string | undefined, targetUsers: string[]): boolean {
...
async function fetchFlagsFromDb(): Promise<Record<string, FeatureFlag>> {
    try {
        ...
        const { data, error } = await supabase
            .from('feature_flags')
            .select('key, name, description, enabled, rollout_percentage, target_users');
...
export const getCachedFlags = cache(async (): Promise<Record<string, FeatureFlag>> => {
    return getAllFlags();
});
...
export async function getFlagsForClient(userId?: string): Promise<Record<string, boolean>> {
    const flags = await getCachedFlags();

...
function replaceRoutePrefix(value: string, routeMap: Readonly<Record<string, string>>): string {
...
export function getMarketPath(path: string, marketCode: MarketCode): string {
  return marketCode === "RO" ? replaceRoutePrefix(path, ROMANIAN_PUBLIC_ROUTE_MAP) : path;
}

...
1. Send the dealer `autoninja-import-template.csv` when they ask for a format.
...
- Brand/model text may need manual matching to the current AutoNinja taxonomy.

Files:

- `autoninja-import-template.csv`: empty template for dealers.
- `autoninja-import-example.csv`: filled CSV example.
- `autoninja-import-example.json`: filled JSON example.
- `autoninja-import-example.xml`: filled XML example.

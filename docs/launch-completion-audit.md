...
- `npm run test:db:rls -- supabase/tests/billing-checkout-atomicity.test.sql`: blocked because Docker Desktop is not running its Linux engine (`dockerDesktopLinuxEngine` pipe missing).
...
- SEO remains launch-blocked until noindex/indexing, Vercel public URL env confirmation, preview smoke, and production smoke are resolved.

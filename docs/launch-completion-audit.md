...
| Signup | Page/UI checks plus mocked register/resend POST route coverage. Real Resend/Gmail delivery of `Potvrdenie registrácie - Autobazar123` to a Gmail launch alias passed on 2026-06-20, and temporary launch-test users were deleted. | Partial | Real browser signup submit plus confirmation-link login still missing. |
...
- Real signup confirmation-link login check.
- Real password reset emailed-link password update check.

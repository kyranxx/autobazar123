...
- [ ] Add listing works. `Partial`: protected guest redirect works, and account ads POST route behavior has mocked local coverage for draft creation, free auto-publish, and failed publish cleanup. Authenticated browser creation still needs a real test-account run. No listing was created in this pass.
- [ ] Edit/manage own listing works. `Blocked`: protected guest redirect works, and route-level quick-edit / feature-action behavior has mocked local coverage. The configured E2E account has no ads, so real dashboard edit/top/sold controls were skipped. Read-only DB audit found one non-admin seller with an ad, but no local credentials are configured for it.
...
- [ ] Payment flow works if paid features are enabled. `Partial`: `/platba/uspech` and legacy `/kredity/uspech` route behavior are verified locally, checkout-status GET behavior has mocked local coverage, and Stripe checkout/webhook POST behavior has mocked local coverage. Paid dashboard/dealer checkout checks were skipped because the configured account has no ads and no dealer role. DB audit found 0 dealer owners. Real Stripe checkout and live webhook delivery still need verification.
- [ ] Admin area still works. `Partial`: guest/admin guardrails work; the configured admin account reached the admin dashboard, the non-dealer dealer prompt was verified, and the dealer verification request API has mocked coverage for owner-scoped state, duplicate pending guard, verified/missing dealer rejection, and request creation. Non-admin admin denial still needs a non-admin account.
...
- [ ] No open P0/P1 defect in auth, payment, search, listing lifecycle, or admin permissions. `Blocked`: no current local P0/P1 found in available checks, but browser listing creation/editing, real inquiry delivery, real checkout/webhook delivery, dealer billing, non-admin admin denial, password reset, and signup still need full real-account coverage.

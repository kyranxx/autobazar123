...
- [ ] Vercel env/build preflight is green. `Blocked on cloud verification`: public Supabase/App URL values were fixed and sensitive server envs were re-added where local source values exist. Latest `npx vercel env ls preview` / `npx vercel env ls production` checks show expected env names exist, including maintenance bypass, Upstash, Stripe, Supabase, Resend/email, Algolia, cron, and Cloudflare keys. Local `vercel env run` cannot prove sensitive Preview/Production values after they are marked sensitive, so a real Vercel cloud Preview build/smoke is still required to verify runtime-sensitive envs and Production Stripe live values.

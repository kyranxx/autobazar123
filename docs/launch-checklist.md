...
- [ ] No open P0/P1 defect in auth, payment, search, listing lifecycle, or admin permissions. `Blocked`: live anon Supabase currently reads raw `profiles` and `dealers` because `20260618174500_harden_profile_dealer_public_reads.sql` is local-only. Also still open: real checkout/webhook delivery and real payment email delivery.

...
- [ ] Vercel env/build preflight is green. `Blocked`: Preview public Supabase/App URL values were fixed and now pull without literal `\r\n`, but `npx vercel env run -e preview -- npm run build` still fails at `/predajca/[slug]` because `SUPABASE_SERVICE_ROLE_KEY` is empty/unusable. `npx vercel env run -e production -- npm run build` fails `check:prod-rate-limit-env` because `UPSTASH_REDIS_REST_TOKEN`, `RESEND_API_KEY`, and `ALGOLIA_SYNC_SECRET` are missing/unusable.
...
- [ ] Maintenance bypass still works for us. `Blocked`: local token helper, unlock route, and proxy host behavior have unit coverage, and the unlock route no longer accepts the legacy `MAINTENANCE_PASSWORD` alias. Current secret-safe Vercel pulls show Preview lacks `MAINTENANCE_UNLOCK_PASSWORD` / `MAINTENANCE_BYPASS_SECRET`, and Production has those keys but they pull as empty/unusable, so live bypass cannot be smoked until the envs are re-created and deployed.
...
- [ ] No open P0/P1 defect in auth, payment, search, listing lifecycle, or admin permissions. `Blocked`: live anon Supabase currently reads raw `profiles` and `dealers` because `20260618174500_harden_profile_dealer_public_reads.sql` is local-only. Also still open: paid checkout completion, live webhook delivery, billing side effects, and real payment email delivery.
...
- Vercel secret/env preflight remains blocked: Preview build with `npx vercel env run -e preview -- npm run build` compiles and typechecks, then fails at `/predajca/[slug]` because `SUPABASE_SERVICE_ROLE_KEY` is empty/unusable. Production build with `npx vercel env run -e production -- npm run build` fails `check:prod-rate-limit-env` because `UPSTASH_REDIS_REST_TOKEN`, `RESEND_API_KEY`, and `ALGOLIA_SYNC_SECRET` are missing/unusable. Production `EMAIL_FROM` and `EMAIL_REPLY_TO` still contain literal `\r\n`; Vercel CLI refused updating those sensitive vars without delete/recreate.

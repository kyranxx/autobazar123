...
- [ ] Sign up works. `Partial`: page opens, UI/a11y checks passed, register/resend POST routes have mocked local coverage, and 2026-06-20 real Resend/Gmail delivery of `Potvrdenie registrácie - Autobazar123` to a Gmail launch alias passed. Real browser signup submit plus confirmation-link login still need a full browser run.
...
- [ ] Password reset works. `Partial`: page opens, UI/a11y checks passed, password-reset POST route has mocked local coverage for recovery link generation and queueing, account password recovery POST has mocked local coverage for token verification, password update, service-role recovery-session revocation, and benign consumed-session cleanup, the 2026-06-19 release gauntlet verified a real Supabase recovery token in the browser, and 2026-06-20 real Resend/Gmail delivery of `Obnovenie hesla - Autobazar123` passed. Real emailed-link password update from the inbox still needs a browser run.
...
- [ ] Maintenance bypass still works for us. `Blocked`: local token helper, unlock route, and proxy host behavior have unit coverage, and the unlock route no longer accepts the legacy `MAINTENANCE_PASSWORD` alias. Safe Vercel Production/Preview env pulls showed the historical leaked value is not configured, but `MAINTENANCE_UNLOCK_PASSWORD` and `MAINTENANCE_BYPASS_SECRET` are missing in both targets, so live bypass cannot be smoked until those envs are set.
...
- [ ] No open P0/P1 defect in auth, payment, search, listing lifecycle, or admin permissions. `Blocked`: live anon Supabase currently reads raw `profiles` and `dealers` because `20260618174500_harden_profile_dealer_public_reads.sql` is local-only. Also still open: real checkout/webhook delivery, signup confirmation-link login, and reset emailed-link password update.
...
- Vercel Production and Preview `NEXT_PUBLIC_APP_URL` were verified by temporary env pulls and fixed to `https://www.autobazar123.sk`; fresh pulls confirmed both targets match `www` and no literal `\r\n` escape remains.

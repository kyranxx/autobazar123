...
- Current-commit migration preflight passed from a fresh throwaway worktree at `C:\Users\User\Desktop\Projects\autobazar123-launch-preflight-20260620-01` on commit `7426f49`: `npx supabase --workdir <verify-worktree> migration list` showed the same three launch-critical local-only migrations, and `npx supabase --workdir <verify-worktree> db push --dry-run --include-all` listed only those three migrations. The throwaway worktree was removed. The persistent `autobazar123-launch-db` folder was later observed stale at `99efd14`, so refresh/recreate it before any deploy or remote DB push.

...
- [ ] Vercel env/build preflight is green. `Blocked`: app-side local Next build now passes after moving dealer pages plus pricing/taxonomy API routes behind `connection()` request boundaries, but local Vercel Preview packaging still fails on static-PPR route `/audi/a1` with `Unable to find lambda for route`. This matches the open Vercel/Next 16 Cache Components static-PPR builder issue where `routesManifest.ppr.chain.headers.next-resume=1` is present. Public Supabase/App URL values were fixed and sensitive server envs were re-added where local source values exist; sensitive Preview/Production values still require real cloud build/runtime smoke.
...
- Clean-worktree dry-run evidence proved the safe path: after the already-remote `20260619120000_add_vehicle_taxonomy_candidates.sql` history file was present locally, `npx supabase --workdir C:\Users\User\Desktop\Projects\autobazar123-launch-db db push --dry-run --include-all` listed only the three launch-critical migrations and did not list `20260619214332_add_vehicle_taxonomy_metadata.sql`. That historical worktree was later removed; recreate a fresh clean worktree and repeat the dry-run before any remote migration.
- `scripts/check-launch-migration-worktree.mjs` now provides an executable guard for the clean launch worktree before Supabase dry-runs or pushes. It fails on dirty `supabase/migrations`, missing required launch migrations, or the unrelated taxonomy metadata migration unless explicitly overridden.
- Guard evidence: `node --test scripts/check-launch-migration-worktree.test.mjs` passed 6/6; the guard fails as expected in the current dirty main worktree and passes against a fresh clean throwaway worktree, which was removed after verification.
...
- Current-commit migration preflight passed from a fresh throwaway worktree at `C:\Users\User\Desktop\Projects\autobazar123-launch-preflight-20260620-01` on commit `7426f49`: `npx supabase --workdir <verify-worktree> migration list` showed the same three launch-critical local-only migrations, and `npx supabase --workdir <verify-worktree> db push --dry-run --include-all` listed only those three migrations. The throwaway worktree was removed. The persistent `autobazar123-launch-db` folder was later observed stale/dirty at detached commit `c978f5c`; after preserving the only unique test mock improvements, it was removed. Create a fresh clean worktree before any deploy or remote DB push.
...
- Local Vercel packaging is still blocked: both `npx vercel build --target=preview --yes` and `npx vercel@54.14.2 build --target=preview --yes` failed on `/audi/a1` with `Unable to find lambda for route`. Do not mark Preview deployment ready or weaken pSEO PPR behavior until the builder issue is resolved or an owner-approved tradeoff is chosen.
- Fresh clean-worktree recheck at commit `292bcd4` reproduced the same `/audi/a1` failure with latest npm `vercel@54.14.2`. `npm view vercel dist-tags` showed `latest=54.14.2` and `canary=51.7.0`; there is no newer npm CLI dist-tag to try. The generated `.next/routes-manifest.json` contains `ppr.chain.headers.next-resume=1`, matching the upstream static-PPR issue.
- Dirty taxonomy/discovery lane recheck: `npx vitest run src/lib/vehicle-taxonomy/public.test.ts src/lib/vehicle-taxonomy/candidates.test.ts src/lib/vehicle-taxonomy/autobazar-eu.test.ts src/lib/vehicle-taxonomy/mobile-de.test.ts src/lib/vehicle-taxonomy/otomoto.test.ts` passed 5 files / 10 tests after adding active-only public taxonomy read coverage. `git diff --check`, `npm run typecheck`, and `npm run lint` passed. Keep `20260619214332_add_vehicle_taxonomy_metadata.sql` out of the launch-critical remote DB push unless explicitly approved.
...
- Vercel secret/env preflight remains blocked until cloud verification: Preview server envs were re-added from local source values, including Stripe test keys, Supabase service role, Resend, Algolia, email, cron, Cloudflare, and maintenance bypass values. Production non-payment server envs were re-added from local source values. Latest `npx vercel env ls preview` / `npx vercel env ls production` checks show expected env names exist in both targets, including Upstash and Stripe names, but local `vercel env run` is not authoritative proof for sensitive deployed runtime values. Upstash and Production Stripe values still need cloud smoke or provider/dashboard confirmation.

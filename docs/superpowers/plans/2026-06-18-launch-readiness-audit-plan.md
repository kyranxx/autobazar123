...
- Last deployed clean source `2297260` passed `npm run check:deploy-source-readiness`, `npm run test:payments-release` 6 files / 53 tests, and `npm run check:launch-blockers:full -- --allow-extra-worktrees`. Latest clean reviewed source `d4bdccf` now passes the full rollup except for missing Turnstile Vercel env metadata names.
...
- Fresh local real Stripe dealer top-up checkout creation smoke passed: `PLAYWRIGHT_CHROMIUM_CHANNEL=chrome npm run smoke:stripe-dealer-topup` logged in with the configured dealer account on `localhost`, created a test-mode `dealer_100` Checkout Session, verified the `billing_checkout_sessions` row, expired the Stripe session, deleted 1 checkout row and 1 idempotency row, left 0 webhook-log residue, and proved dealer prepaid balance stayed unchanged. Helper verification passed first: `node --check scripts/smoke-stripe-dealer-topup.mjs`, `npm run test:stripe-dealer-topup-smoke-script` 8/8, `npm run test:payments-release` 6 files / 53 tests, and `npm run test:security:release-gate`.
- Fresh protected Preview real Stripe dealer top-up checkout creation smoke passed through a temporary Vercel protected-preview access URL: `TEST_URL=<protected-preview-share-url> PLAYWRIGHT_CHROMIUM_CHANNEL=chrome npm run smoke:stripe-dealer-topup` logged in with the configured dealer account on Preview, created a test-mode `dealer_100` Checkout Session, verified the checkout DB row, expired the Stripe session, deleted 1 checkout row, 1 idempotency row, and 1 webhook log row, and proved dealer prepaid balance stayed unchanged. The share token was not stored.
- Fresh current-main `npm run check:launch-blockers -- --allow-extra-worktrees` after the dealer top-up smoke command still blocks only on expected current-main posture: dirty deploy source from the separate taxonomy lane, missing Turnstile Vercel env metadata names, and current-tree launch migration safety from the separate taxonomy metadata migration. Package-script tracking passed with 115 local paths and the upload release suite passed 19/19.
- Fresh read-only Production external browser audit found a deployed mobile `_rsc` prefetch/rate-limit issue: the capped external audit failed 7/40 route/viewport checks because protected account/dealer `_rsc` prefetches returned 429 on mobile. Clean reviewed source `d4bdccf` has a local RED/GREEN proxy fix so protected-route `_rsc` prefetches no longer consume the protected-route rate-limit budget before redirect/RBAC handling. Not deployed yet.
...
- Real Preview Stripe payment success and failed-payment paths now pass for seller listing actions. Local and protected Preview real Stripe dealer top-up checkout creation now pass with cleanup and unchanged dealer balance. Broader deployed dealer topup paid/failed variants remain open.
...
- A Chrome Turnstile dashboard attempt reached the Cloudflare Turnstile account page, but Chrome automation was blocked by an open extension popup before any widget/env change could be made. The Turnstile tab was left open as a handoff. No Cloudflare or Vercel settings were changed.
...
- Clean reviewed source `d4bdccf` also has a local fix for deployed mobile `_rsc` prefetch 429 noise: `npx vitest run src/proxy.test.ts` passes 19/19 after the new regression first failed; the fresh full rollup also passed security, typecheck, lint, build, and Vercel Preview local build preflight. The fix is not deployed.
...
- Dirty taxonomy/discovery work remains in the main worktree and is not part of the launch-critical path. Fresh recheck passed focused taxonomy tests 7 files / 18 tests, `npm run typecheck`, `npm run lint`, and `git diff --check`; no-mode scripts fail closed; read-only promotion dry-run reported 0 approved candidates; Wikidata `--dry-run --limit=1` returned 0 candidates; Autobazar.eu Tesla `--dry-run` returned 4 model candidates; Otomoto `--dry-run` returned 115 candidates. Current-main deploy-source and migration-worktree guards still block this dirty lane from launch deploy/DB push. The lane adds external discovery scripts, candidate promotion, and local-only migration `20260619214332_add_vehicle_taxonomy_metadata.sql`; keep it out of future launch DB pushes unless explicitly approved as a separate feature.

...
- Current-commit migration preflight evidence: a fresh throwaway worktree at `C:\Users\User\Desktop\Projects\autobazar123-launch-preflight-20260620-01` on commit `7426f49` passed `npx supabase --workdir <verify-worktree> migration list` and `npx supabase --workdir <verify-worktree> db push --dry-run --include-all`; the dry-run listed only those same three launch-critical migrations and the throwaway worktree was removed. The persistent `autobazar123-launch-db` folder was later observed stale at `99efd14`, so refresh/recreate it before deploy or remote DB push.
